...
export const LibraryPage = () => {
  ...
  const handleInputSearch = (e: any) => {
    if (inputValue.length === 0) setIsInputLoading(false);
    else setIsInputLoading(true);
  };
    ...
    <section className="flex flex-col items-center w-full max-w-4xl gap-6 px-4 py-4 mx-auto ">
      ...
      <div className="flex flex-col items-center w-full gap-6">
        ...
        <div className="inline-flex items-center w-full max-w-md gap-2">
          <InputComponent inputPlaceholder="Search by keywords, topic or RSS page." completeInput={false} inputType="search" inputValue={inputValue} onChange={(e: any) => handleInputChange(e)} />
          <ButtonComponent btnText="Search" btnType="submit" btnIcon={<TbSearch />} btnOnClick={() => handleInputSearch()}/>
        </div>
      </div>    
      ...
      {/* Suggestions if input is not empty & api response */}
